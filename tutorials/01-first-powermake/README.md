...
First of all, make sure PowerMake is up-to-date, new versions are often out every two weeks.  
...
We will create a very small project that looks like this:
...
> [!NOTE]  
> If you've looked carefully the commands generated by PowerMake, you may be wondering why PowerMake is using the `g++` linker while we are only compiling a C file.  
...
An easier way to run your program is to use the builtin `test` callback in powermake.
...
This function can be named as you like, but should always take a `config` parameter.  
...
The first line enables most warnings for our compilation, we may want to keep that, it will never hurt.  
...
Do you feel a little more comfortable with the basic makefile ?
...
### [-> Next tutorial (Set up your IDE)](../02-ide-setup/README.md)
