...
def run(target_name: str, *, build_callback: callable, clean_callback: callable = default_on_clean, install_callback: callable = default_on_install, args_parsed: argparse.Namespace = None):
    ...
    if args_parsed.action == "clean" or args_parsed.clean:
        clean = True
    if args_parsed.action == "build" or args_parsed.build or args_parsed.rebuild:
        build = True
    if args_parsed.action == "install" or args_parsed.install is not False or args_parsed.i:
        install = True
    ...
    if install:
        if args_parsed.action == "install":
            install_callback(config, args_parsed.install_location)
        else:
            install_callback(config, args_parsed.install or None)
    ...
    if args_parsed.get_lib_build_folder:
