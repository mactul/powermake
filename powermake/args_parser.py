...
class ArgumentParser(argparse.ArgumentParser):
    def __init__(self, prog: T.Union[str, None] = None, description: T.Union[str, None] = None, **kwargs):
        if prog is None:
            prog = "powermake"
...
def generate_config(target_name: str, args_parsed: T.Union[argparse.Namespace, None] = None):
...
def run_callbacks(config: Config, *, build_callback: Callable[[Config], None], clean_callback: Callable[[Config], None] = default_on_clean, install_callback: Callable[[Config, T.Union[str, None]], None] = default_on_install):
    ...
    if config._args_parsed is None:
        raise RuntimeError("powermake.run_callbacks should only be run with a config generated by powermake.generate_config")
...
def run(target_name: str, *, build_callback: Callable[[Config], None], clean_callback: Callable[[Config], None] = default_on_clean, install_callback: Callable[[Config, T.Union[str, None]], None] = default_on_install, args_parsed: T.Union[argparse.Namespace, None] = None):
