...
def _get_last_compilation_unit(config: Config) -> T.Union[str, None]:
    try:
        file = open(os.path.join(config.info_build_directory, "last_compilation_unit"), "r")
...
def run_callbacks(config: Config, *, build_callback: T.Callable[[Config], None], clean_callback: T.Callable[[Config], None] = default_on_clean, install_callback: T.Callable[[Config, T.Union[str, None]], None] = default_on_install, test_callback: T.Callable[[Config, T.List[str]], None] = default_on_test) -> None:
    ...
    if config._args_parsed.compilation_unit is not None and _get_last_compilation_unit(config) == config._args_parsed.compilation_unit:
        print_debug_info("Makefile already run during this compilation unit", config.verbosity)
    ...
    if config._args_parsed.compilation_unit is not None:
        makedirs(config.info_build_directory)
        file = open(os.path.join(config.info_build_directory, "last_compilation_unit"), "w")
